...
(defn ep->map [in] (loop [tagmap {} nodes in]
                     (cond
                       ...
                       (-> (first nodes) (:attrs) (seq))
                       (let [fn (first nodes) _ (rest nodes)]
...
(defn ingest-channel
  ...
  (db/upsert-channel con {:itunesID (long itunesID)
                          :feedUrl feedUrl
                          :title (:title feed)
...
(defn ingest-episode
  [itunesID ep con]
  #_{:clj-kondo/ignore [:unresolved-var]}
  (db/upsert-episode con {:channel (long itunesID)
                          :title (:title ep)
                          :itunesTitle (:itunes:title ep)
                          :fileUrl (-> ep (:enclosure) (:attrs) (:url))
                          :fileSize (try (-> ep (:enclosure) (:attrs) (:length) (Long/parseLong)) (catch Exception _ nil))
                          ...
                          :itunesDuration (when (:itunes:duration ep)
     ...
     (let [feed (get-feed feedUrl)]
       (jdbc/with-transaction [tx db/db]
         [(ingest-channel itunesID (ep->map feed) feedUrl tx)
          ...
          (when (and rank fetchedAt)
            #_{:clj-kondo/ignore [:unresolved-var]}
            (db/add-to-topchart tx
                                {:channel (long itunesID)
                                 :rank rank
                                 :fetchedAt fetchedAt}))])) (catch Exception e (do (println itunesID feedUrl (:cause e)) e)))))
...
(defn chan-fetchorreadcache [row validdate]
  (if (jt/before? (jt/instant (:lastfetched row)) validdate)
    (do (add-channel (:itunesid row) (:feedurl row))
        (first #_{:clj-kondo/ignore [:unresolved-var]}
         (db/channels-by-ids db/db {:ids [(:itunesid row)]})))
    row))

(defn getchans-or-fetch [ids]
  (let [rows (fliplist #_{:clj-kondo/ignore [:unresolved-var]}
              (db/channels-by-ids db/db {:ids ids}) :itunesid)
      ...
      (if (seq pods)
        (if (contains? resids (first pods))
          ...
          (let [lookup (first (itunes/lookup-ids [(first pods)]))]
            ...
            (recur (conj ret
                         (first #_{:clj-kondo/ignore [:unresolved-var]}
                          (db/channels-by-ids db/db {:ids [(first pods)]})))
                   (rest pods))))
        ret))))

(defn geteps-or-fetch [id]
  (let [oldrow (first #_{:clj-kondo/ignore [:unresolved-var]}
                (db/channels-by-ids db/db {:ids [id]}))]
    (cond (not oldrow)
